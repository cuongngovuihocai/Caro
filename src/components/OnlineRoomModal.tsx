...
export const OnlineRoomModal: React.FC<OnlineRoomModalProps> = ({
  ...
  const handleCopyCode = () => {
    ...
    const roomUrl = `${window.location.origin}${window.location.pathname}?room=${roomState.id}`;
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-amber-950 tracking-wider font-mono mt-1 flex items-center gap-2">
              ...
              <button
                ...
                className="p-1.5 px-2.5 rounded-lg bg-amber-200 hover:bg-amber-300 text-amber-900 border border-amber-300 transition-colors cursor-pointer text-xs flex items-center gap-1 font-sans"
                title="Sao chép link phòng chơi"
              >
                {copiedCode ? <Check className="w-4 h-4 text-emerald-700" /> : <Copy className="w-4 h-4" />}
                <span>{copiedCode ? 'Đã chép link!' : 'Sao chép link'}</span>
              </button>
            </h2>
          </div>
...
export default OnlineRoomModal;
